...
namespace SharpQuake.Game.Data.Models
{
	public class BrushModelData : ModelData
    {
        ...
        private void BuildSurfaces2()
        {
            ...
            lmTexture = BaseTexture.FromBuffer( _device, "LM", NewLightData, LIGHTMAP_WIDTH, LIGHTMAP_HEIGHT, false, false, "GL_LINEAR" );
        ...
        private GLPoly BuildPoly( MemorySurface fa )
        {
            ...
            AllocBlock( LightmapBlockWidth, LightmapBlockHeight, out fa.light_s, out fa.light_t );

            var length = LightmapBlockWidth * LightmapBlockWidth;

            if ( fa.sampleofs >= LightData.Length || fa.sampleofs + length >= LightData.Length )
            {
            ...
            else
            {
                ...
                //var texelsOffset = fa.sampleofs;

                if ( LightmapTexels != null && LightmapTexels.Length > 0 )
                {
                    for ( var Y = 0; Y < LightmapBlockHeight; Y++ )
                    {
                        for ( var X = 0; X < LightmapBlockWidth; X++ )
                        {
                            ...
                            if ( fa.sampleofs == -1 )
                                NewLightData[ImageIndex] = ( UInt32 ) Color.White.ToArgb( );
                            else
           ...
           // poly.next = fa.polys;
            ...
            for ( var i = 0; i < lnumverts; i++ )
            {
                ...
                t += fa.light_t * 16;
                t += 8;
                t /= LIGHTMAP_HEIGHT * 16;// LIGHTMAP_HEIGHT * 16;
